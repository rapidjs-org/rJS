...
export class Printer {
	...
	private static readonly brandSequence: string
		= Printer.format(` ${
			Printer.format("r", [ Printer.escapes.PRIMARY_COLOR_FG ], 39)
		}${
			Printer.format("JS", Printer.escapes.DARK_COLOR_FG, 39)
	...
	public static indentWithBrandSequence(sequence: string): string {
		return `${
			" ".repeat(Printer.brandSequence.replace(/\x1b\[\d+(;\d+)*m/g, "").length + 1)
		}${sequence}`;
	}
