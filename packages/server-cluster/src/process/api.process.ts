...
process.once("message", async (workerData: IAdapterConfiguration) => {
	...
	function writeResponse(sResPartial: Partial<ISerialResponse>, socket?: Socket, keepalive: boolean = true): Promise<Partial<ISerialResponse>> {
		return new Promise((resolve, reject) => {
			...
			socket.write(body, (err?: Error) => {
				...
				(
					[ (sResPartial.headers ?? {})["Connection"] ?? "" ]
					.flat()[0]
					.toString()
					.toLowerCase() === "keep-alive"
				)	// keep-alive ?
				? respond()
				: socket.end(respond);
			});
	...
	.then(async (handler: TAdapter) => {
		process.on("message", async (sReq: ISerialRequest, socket?: Socket) => {
			try {
				process.send(await writeResponse(await handler(sReq), socket));
			} catch(err) {
				process.send(await writeResponse({
